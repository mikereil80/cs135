...
(define (CRT-exists? tuplelist)
  (if (eq? (length tuplelist) 1) #t
      (CRTexisthelp (cdr tuplelist) (car (cdr (car tuplelist))) tuplelist)))

(define (CRTexisthelp tuplelist first list2)
  (if (eq? tuplelist '()) (CRT-exists? (cdr list2))
  (if (eq? (EuclidAlgo (car (cdr (car tuplelist))) first) 1) (CRTexisthelp (cdr tuplelist) first list2)
      #f)))
...
(define (CRT-helper tuplelist m)
  (if (eq? tuplelist '()) 0
      (+ (* (* (car (car tuplelist)) (quotient m (car (cdr (car tuplelist))))) (mul_inv (quotient m (car (cdr (car tuplelist)))) (car (cdr (car tuplelist)))))
         (CRT-helper (cdr tuplelist) m))))
...
(define (CRT tuplelist)
  (if (eq? (CRT-exists? tuplelist) #f) -1
