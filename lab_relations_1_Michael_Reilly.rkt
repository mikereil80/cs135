...
(define (reflexive? relation n)  
    (if (equal? (intersection relation (id n)) (id n)) #t
        #f))
...
(define (flip-pairs relation)
  (if (equal? relation '()) '()
  (if (equal? (cardinality relation) 1) (list (reverse (car relation)))
...
(define (symmetric? relation)
   (if (equal? (intersection relation (flip-pairs relation)) relation) #t
       #f))
