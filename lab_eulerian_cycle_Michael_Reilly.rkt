...
(define (degree relation vertex)
  (if (equal? relation '()) 0
      (if (equal? (car (car relation)) vertex)
          (if (equal? (car (cdr (car relation))) vertex) (+ 2 (degree (cdr relation) vertex))
...
(define (maxVertex relation)
   (if (equal? relation '()) 1
       (if (equal? (length relation) 1) (max (car (car relation)) (car (cdr (car relation))))
           (max (car (car relation)) (maxVertex (cdr relation))))))

(define (max number1 number2)
  (if (eq? number1 number2) number1
...
(define (has-eulerian-cycle? relation)
   (vertex (maxVertex relation) relation))

(define (vertex number relation)
  (if (equal? number 0) #t
      (if (equal? (degree relation number) 1) (vertex (- number 1) relation)
          (if (equal? (modulo (degree relation number) 2) 0) (vertex (- number 1) relation)
              #f))))
