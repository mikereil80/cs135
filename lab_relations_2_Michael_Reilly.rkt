...
(define (composite  relationOuter relationInner)
  (if (equal? relationInner '()) '()
      (if (equal? (related-to (car (cdr (car relationInner))) relationOuter) '()) (composite relationOuter (cdr relationInner))
          (append (list (append (list (car (car relationInner))) (related-to (car (cdr (car relationInner))) relationOuter))) (composite relationOuter (cdr relationInner))))))
...
(define (power relation k)
  (if (eq? k 0) '()
      (if (eq? k 1) relation
      (store relation (composite relation (power (store relation relation) (- k 1)))))))

(define (store relation1 relation2)
  relation2)
...
(define (transitive-closure relation)
  (union relation (help-power relation relation (length relation))))

(define (help-power relation1 relation2 k)
  (if (eq? k 0) relation2
  (union (power relation2 k) (help-power relation1 relation2 (- k 1)))))
...
(define (transitive? relation)
  (if (equal? relation '()) #t
   (if (equal? (element? (reverse (car relation)) relation) #f) #f
       (transitive? (remove (reverse (car relation)) (cdr relation))))))
...
(define (EQ-relation? relation n)
  (if (equal? (transitive? relation) #t)
      (if (equal? (symmetric? relation) #t)
          (if (equal? (reflexive? relation n) #t) #t
          #f)
      #f)
  #f))
