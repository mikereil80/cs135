...
(define (subset? setA setB)
  (equal? setA (intersection setA setB)))
...
(define (set-equal? setA setB)
  (equal? (make-set (append setA setB)) setB))
...
(define (set-difference listA listB)
  (make-set (if (equal? listA '()) '()
  (if (element? (car listA) (intersection listA listB))
...
(define (sym-diff listA listB)
  (make-set (union
        (set-difference listA listB) (set-difference listB listA))))
...
(define (remove element set)
  (make-set (if (element? element set)
      (set-difference set (cons element '()))
            set)))
